...
use facet_core::{
    ConstTypeId, Facet, NumericType, PointerType, PrimitiveType, SequenceType, Shape, SliceType,
    TextualType, Type, UserType, ValuePointerType,
};
use std::sync::RwLock;
...
use encoder::Encoder;
use primitive::PrimitiveEncoder;
use slice::SliceEncoder;
use strided::{StridedEncoder, StructEncoder};
...
mod tests {
    ...
    fn bench_mesh_serde_bitcode(b: &mut Bencher) {
        let v = mesh();
        b.iter(|| {
            black_box(bitcode::serialize(black_box(&v)))
        })
    }
